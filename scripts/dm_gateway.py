...
from src.api.dm_api import make_dm_topic
...
app = FastAPI(title="RapidLink Gateway", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def index() -> HTMLResponse:
    ...
    return HTMLResponse("<h1>RapidLink Gateway</h1><p>UI not found. Place index.html under public/.</p>")
...
def normalize_dm_identity(value: str) -> str:
    return " ".join(value.strip().split()).lower()


def build_dm_conversation_id(user_a: str, user_b: str) -> str:
    left = normalize_dm_identity(user_a)
    right = normalize_dm_identity(user_b)
    if not left or not right:
        raise ValueError("Both participants are required")
    if left == right:
        raise ValueError("Participants must be different")
    return make_dm_topic(left, right)


def parse_dm_conversation_id(convo_id: str) -> tuple[str, str] | None:
    parts = convo_id.split(":")
    if len(parts) != 3 or parts[0] != "dm":
        return None
    left = normalize_dm_identity(parts[1])
    right = normalize_dm_identity(parts[2])
...
def validate_dm_sender(convo_id: str, sender: str | None) -> None:
    if not sender:
        return
    participants = parse_dm_conversation_id(convo_id)
    if not participants:
        return
    normalized_sender = normalize_dm_identity(sender)
    if normalized_sender not in participants:
        raise HTTPException(status_code=400, detail="Sender must match one of the two conversation participants")


@app.post("/dm/{convo_id}")
async def send_message(convo_id: str, payload: DMRequest) -> Dict[str, Any]:
    text = payload.message.replace("\n", " ").strip()
    validate_dm_sender(convo_id, payload.sender)
    if payload.sender:

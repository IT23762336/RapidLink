...
# Ensure repo root is on sys.path so `from src...` works when running this file directly
...
def demonstrate_time_sync():
    ...
    try:
        ...
        print(f"\nSynchronized time (client): {sync_time:.6f}")
    finally:
        server1.stop()
        server2.stop()
...
def demonstrate_bounded_reordering():
    """Demonstrate bounded reordering for message delivery."""
    print("\n=== BOUNDED REORDERING DEMONSTRATION ===\n")
    reordering = BoundedReordering(max_delay_ms=100)
    
    # Simulate receiving messages slightly out of order
    now = time.time()
    msgs = [
        {"id": "1", "timestamp": now - 0.2},
        {"id": "2", "timestamp": now - 0.05},
        {"id": "3", "timestamp": now}
    ]
    
    for m in msgs:
        ready = reordering.add_message(m)
        if ready:
            print("Ready:", [x["id"] for x in ready])
        time.sleep(0.06)


def demonstrate_message_ordering():
    """Demonstrate message ordering with logical timestamps."""
    print("\n=== MESSAGE ORDERING (LOGICAL) ===\n")
